...
## 5. テスト駆動（Testing First）

**原則**: 実装とテストは常にセットで納品する。テストなき実装は未完成とみなす。
...
**チェックリスト**:
- [ ] 新規ロジックに対応するユニットテストが存在するか
- [ ] ランタイム動作が統合テストで検証されているか
- [ ] ビルドがエラーなく通過するか
- [ ] テスト不要の場合、理由がチケットに明記されているか

### Unity 固有ルール

| テスト種別 | 対象 | 必須度 |
|-----------|------|-------|
| EditMode テスト | ロジック検証（MonoBehaviour不要） | 必須 |
| PlayMode テスト | ランタイム動作検証 | 必須 |
| ビルド検証 | コンパイルエラーゼロ | 必須 |
| 手動検証 | UI/UX、エディタ拡張 | 推奨 |
