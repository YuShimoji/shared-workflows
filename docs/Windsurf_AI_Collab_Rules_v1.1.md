...
- 進捗共有は「目的・現状・次のアクション」を含むことを基本とし、それ以外の表現方法（物語調/要約/視覚的リスト等）は `report_style` に従い自由に選択する。
- 応答モード: 「標準モード」（テンプレート遵守）と「フレキシブルモード」（効果優先）を状況に応じ自動選択。
- フィードバックループ: 応答後に人間から効果評価を収集し、ルール改善に反映。
- レポートスタイルの既定値や禁止表現は `REPORT_CONFIG.yml` で上書き可能。
...
- レポートスタイル: `report_style` フィールドに `standard`/`concise`/`narrative`/`creative` 等を明示し、AIの報告トーンと構造を指示する。
...
- `REPORT_CONFIG.yml` で以下を指定可能。
  - `default_style`: `AI_CONTEXT.md` 未指定時の既定スタイル。
  - `banned_phrases`: 使用禁止ワード/表現。
  - `style_presets`: スタイル名ごとの推奨テンプレ（見出し構成・要素）。
- `scripts/report-style-hint.js` をCIや対話起動時に実行すると、スタイルに応じたガイドラインを `REPORT_HINT.md` として生成し、AIへ渡せる。
- 変更後は `AI_CONTEXT.md` または対話冒頭で現在の `report_style` を再宣言し、整合性を保つ。
...
- **スタイル別の例**:
  - `concise`: 「APIバグ修正中。原因特定済。次はユニットテスト。」
  - `narrative`: 「今朝はログ解析から始め、根本原因を突き止めました。次にテストで検証します。」
  - `creative`: 「進捗は快調、CIの荒波も乗り越えました。残るは結合テストの帆を張るのみです。」
