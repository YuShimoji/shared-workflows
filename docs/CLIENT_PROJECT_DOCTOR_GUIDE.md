...
**重要**: SSOT ファイル（`Windsurf_AI_Collab_Rules_*.md`）は、shared-workflows サブモジュールのバージョンによって `latest.md` / `v2.0.md` / `v1.1.md` のいずれかが存在します。プロンプトやスクリプトは、この順序でフォールバック参照し、最初に見つかったものを基準として動作します。以下の用途に使用できます:
