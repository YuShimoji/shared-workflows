...
3. todo_list を更新（1つだけ in_progress を維持）
   - `node scripts/todo-sync.js` を実行し、docs/tasks/ → AI_CONTEXT.md（短期: Next）→ Windsurf UI todo_list の順で同期する。**AI_CONTEXT.md に `### 短期（Next）` が無くても自動で挿入されるため、手動編集は不要。** CLI が使えない環境では手動同期してから応答する。
