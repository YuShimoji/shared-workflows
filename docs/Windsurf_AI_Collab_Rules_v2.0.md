...
> **重要**: 本バージョンは v1.1 の問題点（自動PR/マージ直前での停止、不要なコード残留）を根絶するため全面的に再設計されました。
...
2. **ミッション確認**: 現在のミッションと進捗を把握
3. **継続 or 新規判定**: 継続作業なら中断可能点から再開、新規ならIssue作成から開始
...
### 1.4 コマンド実行の事前承認（全コマンド）

- **すべてのターミナルコマンドは事前承認が必須**（read-only を含む）
- 確認の往復を減らすため、AI は次に必要なコマンドを **バッチ（複数本）** で提示し、ユーザーが **一括承認** してから実行する
- AI は承認済みバッチ以外のコマンドを実行してはならない
...
ユーザーは「OK」または「このバッチだけOK」等で承認し、AI は承認範囲内でのみ実行する。
