...
- **テストなき実装は未完成**: Worker が実装を行う場合、対応するテストコード（ユニットテスト / Play Mode テスト / ビルド検証）が存在しなければ DoD 未達とする。
- **テスト計画先行**: P4（チケット発行）で必ず「Test Plan」セクションを含める。テスト対象・テスト種別・期待結果を明記する。
- **Green before Merge**: Worker の納品物は「テスト全通過 + ビルド成功」が確認できなければ DONE にしない。
- **Unity プロジェクトの場合の必須テスト**:
  - EditMode テスト（ロジック検証）
  - PlayMode テスト（ランタイム動作検証）
  - ビルド成功確認（target platform でエラーなし）
- **Web プロジェクトの場合の必須テスト**:
  - ユニットテスト（Jest / Vitest 等）
  - E2E テスト（該当する場合）
...
- P2.5（発散思考）フェーズで集中的に実施し、P3 以降は収束に向かう。
